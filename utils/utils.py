...
def adjust_learning_rate_cifar(args, optimizer, epoch, zero_lr_for_epochs, train_writer):
    """Decay LR in [81 122 151]"""
    lr_schedule = [81, 122, 151]
    lr_decay = 1.
    if epoch in lr_schedule:
        idx = lr_schedule.index(epoch) + 1
        lr_decay = args.lr_decay_scalar ** idx
...
def get_conv_sizes(args, model):
    model.eval() # To make sure BN doesn't complain
    output_sizes = None
